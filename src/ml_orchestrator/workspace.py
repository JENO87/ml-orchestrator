...
class GCPProject:
    ...
    def get_secret(self, secret_name: str, version: str = "latest", fallback_env_var: str | None = None) -> str | None:
        ...
        if self.secret_manager_client is None:
            ...
            secret_path = self.secret_manager_client.secret_version_path(
                project=self.env.secret_manager_project_id or self.env.project_id,  # type: ignore[arg-type]
                secret=secret_name,
    ...
    def get_bucket(self, bucket_name: str | None = None) -> storage.Bucket | None:
        ...
        try:
            bucket = self.storage_client.get_bucket(bucket_name)  # type: ignore[union-attr]
    ...
    def _get_prefix_and_deploy_env(self) -> tuple[str, str]:
        ...
        if self.env.project_id:
            match = re.fullmatch(PROJECT_ID_PATTERN, self.env.project_id)
            if match:
